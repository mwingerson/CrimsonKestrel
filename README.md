...
refactor_on: "Aligning the gateway architecture guide to document the direct python daemon pipeline and stripping legacy Telegraf references"
ai_breadcrumbs:
  - "gateway/crimson_kestrel_daemon.py  # True MQTT-to-InfluxDB direct ingestion flow"
  - "docker-compose.yml                 # Aligned multi-container stack setup"
  - "grafana_dashboard-v7.json          # Live Fahrenheit-standardized variables"
---

> ⚠️ **STALE DOCUMENTATION DEBT**
> **Refactor Trigger:** This README still contains legacy instructions referencing Telegraf container setups. Fully refactor this document when: **Aligning gateway architecture to the direct Python daemon pipeline**.
> **Context to Ingest (Breadcrumbs):** Feed your AI agent these active source files to perform the update with 100% alignment:
> * `gateway/crimson_kestrel_daemon.py` - Contains the current direct MQTT-to-InfluxDB data parser.
> * `docker-compose.yml` - Defines the modern container orchestration layout.
> * `grafana_dashboard-v7.json` - Establishes active measurement keys and Celsius-to-Fahrenheit UI math.
