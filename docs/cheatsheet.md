...
| **Pin 2** | DATA | ESP32-C6 GPIO1 | Single-wire data line. **Requires a physical $4.7	ext{k}\Omega$ to $10	ext{k}\Omega$ pull-up resistor to $V_{CC}$.** |
...
| *(see module datasheet)* | IO1 | GPIO1 | DHT22 single-bus serial data input line (chosen to avoid strapping-pin conflicts). |
...
*   **Recursively Copy Enclosure STL Directory:**
    ```bash
    scp -r ./hardware/enclosure/ <GATEWAY_USER>@<GATEWAY_IP>:~/wrkspc/weather_station_staging/hardware/
    ```
*   **Push Single Configuration File:**
    ```bash
    scp ./docker-compose.yml <GATEWAY_USER>@<GATEWAY_IP>:~/wrkspc/weather_station_staging/
    ```

### B. Pull Files / Folders (Gateway to Local)
*   **Download Completed Telemetry CSV Logs:**
    ```bash
    scp -r <GATEWAY_USER>@<GATEWAY_IP>:~/wrkspc/weather_station_staging/data/ ~/Downloads/
    ```
*   **Pull Single Gateway Daemon Script:**
    ```bash
    scp <GATEWAY_USER>@<GATEWAY_IP>:~/wrkspc/weather_station_staging/gateway/crimson_kestrel_daemon.py ~/Downloads/
    ```
...
3. Issue the local execution escape:
   ```text
   ssh> ! scp -r <GATEWAY_USER>@<GATEWAY_IP>:/home/<GATEWAY_USER>/wrkspc/weather_station_staging/ ~/Downloads/
   ```
...
*   **Tail Live Processing Logs (Last 50 Lines):**
    ```bash
    docker logs crimson_daemon --tail 50
    docker logs crimson_mosquitto --tail 50
    docker logs crimson_influxdb --tail 50
    ```
...
*   **Listen to All Subscribed Topics (Great for confirming packet ingress):**
    ```bash
    docker exec -it crimson_mosquitto mosquitto_sub -h localhost -p 1883 -t "#" -v
    ```
*   **Subscribe Specifically to Aligned Environmental Topics:**
    ```bash
    docker exec -it crimson_mosquitto mosquitto_sub -h localhost -p 1883 -t "telemetry/+/weather" -v
    ```

### B. Simulating Telemetry Ingress (Manual Publishing)
*   **Post Environmental Payload to Broker:**
    ```bash
    docker exec -it crimson_mosquitto mosquitto_pub -h localhost -p 1883 -t "telemetry/weather_node_01/weather" -m '{"client_id":"weather_node_01","environment":{"temperature_c":22.3,"humidity_pct":38.5}}'
    ```
*   **Post Power & Diagnostics Payload to Broker:**
    ```bash
    docker exec -it crimson_mosquitto mosquitto_pub -h localhost -p 1883 -t "telemetry/weather_node_01/system" -m '{"client_id":"weather_node_01","system":{"version":"1.15.4","uptime_ms":3585},"power":{"bus_voltage_v":3.82,"load_current_ma":36.9,"shunt_voltage_mv":3.69}}'
    ```
...
```bash
python3 ~/.arduino15/packages/esp32/hardware/esp32/3.0.0/tools/espota.py -i <NODE_IP> -p 8266 -a <OTA_PASSWORD> -f /tmp/arduino/build/CrimsonKestrel.ino.bin
```
