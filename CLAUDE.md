...
| 9 | README.md had a self-maintained "STALE DOCUMENTATION DEBT" header (YAML frontmatter + warning banner) duplicating this PROJECT REGISTER as a second debt-tracking system — and it had already gone stale itself: it flagged lingering Telegraf references that don't actually exist in the body (Telegraf was fully removed per Register #7), and its breadcrumb pointed at a nonexistent `grafana_dashboard-v7.json` (real file is `gateway/grafana_dashboard.json`). | Done | Removed the frontmatter/banner block from README.md entirely. One tracker (this table) is enough for a project at this stage; a second hand-maintained mechanism just adds another thing to fall out of sync. |
| 10 | Repo-wide doc alignment pass after the README split (Register #9): found real cross-file drift, not just cosmetic staleness. | Done | (1) **Wrong DHT22 pin:** `BOM.md`, `BRINGUP.md`, and `docs/cheatsheet.md` all documented the DHT22 DATA line on GPIO15, but firmware ground truth (`firmware/CrimsonKestrel/CrimsonKestrel.ino`) is `DHTPIN=1` (GPIO1, chosen to dodge strapping-pin conflicts). Worse, `BOM.md`'s own Phase-2 section proposed wiring a future SPS30 fan-control MOSFET to GPIO1 too — a real pin collision if anyone followed the old (wrong) docs and then built Phase 2. Fixed the pin number in all three docs and added an explicit conflict warning in BOM.md's Phase-2 section. (2) **Stale Telegraf/container-name references in `docs/cheatsheet.md`:** `docker logs telegraf` (Telegraf was removed per Register #7) and `docker logs mosquitto`/`docker exec -it mosquitto` used the bare service names instead of the actual `container_name`s in `docker-compose.yml` (`crimson_mosquitto`, `crimson_influxdb`, `crimson_daemon`) — those commands would have failed as written. Fixed to the real container names. (3) **Real credentials/IPs in `docs/cheatsheet.md`:** real gateway IP, real username, real node IP, and what appears to be a real OTA password (`BLACKraptor`) were hardcoded into ~8 example commands. Replaced with placeholders (`<GATEWAY_IP>`, `<GATEWAY_USER>`, `<NODE_IP>`, `<OTA_PASSWORD>`), consistent with the `<GATEWAY_IP>` convention already used in `docs/SETUP.md` and now the README diagram. (4) **Found `docs/doc-debt-policy-v2.md`** — a formal policy mandating the YAML header (`status`/`refactor_on`/`ai_breadcrumbs`) on every `.md` file, which we didn't know about when closing Register #9. In practice only `docs/cheatsheet.md` ever had the header; `BOM.md`, `BRINGUP.md`, and all four ADRs never did. Deleted the policy file and stripped the header from `docs/cheatsheet.md` — same reasoning as Register #9, this table is the one tracker. |
